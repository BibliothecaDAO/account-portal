...
describe("auth rate-limit migration", () => {
  ...
  it(
    "creates the database-backed Better Auth rate-limit model",
    async () => {
      client = new PGlite();
      const migration = await readFile(
        new URL(
          "../../db/migrations/0002_auth_rate_limit.sql",
          import.meta.url,
        ),
        "utf8",
      );
  ...
  it(
    "makes SIWS nonce replacement atomic by enforcing one verification per identifier",
    async () => {
      client = new PGlite();
      await client.exec(`
