...
export function Header() {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 flex shrink-0 items-center px-2 pt-2 transition-[width,height] ease-linear",
      )}
    >
      <div className="realm-header-shell flex h-(--header-height) w-full gap-2 rounded-[1rem] px-2">
        <div
          className={
            `${open ? "w-16 px-1 sm:w-(--sidebar-width)" : "w-(--sidebar-width-icon)"}` +
            " border-border/60 flex items-center justify-center border-r"
          }
        >
          <Link to="/">
            <RWLogo
